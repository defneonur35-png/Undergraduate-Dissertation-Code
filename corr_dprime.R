...
graph1 <- ggplot(corr_data, aes(x = GABA_Delta, y = WM_Score,
  ...
  labs(
    title    = "GABA/NAA Change vs. d prime by Stimulation Condition",
    subtitle = "Pearson r with 95% CI regression band",
...
# --- Graph 2: All conditions overlaid ------------------------
# Shows all three conditions together for direct comparison
...
graph2 <- ggplot(corr_data, aes(x = GABA_Delta, y = WM_Score,
  ...
  theme_classic(base_size = 13) +
  labs(
    title = "GABA/NAA Change vs. d prime",
    x     = expression(Delta * " GABA/NAA (Post – Pre, a.u.)"),
    y     = "d prime"
  ) +
  theme(legend.position = "none")
